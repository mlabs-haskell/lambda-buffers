...
type MonadPrint m = LV.MonadPrint m RsSyntax.QValName
...
caseIntERef = (RsSyntax.MkCrateName "lbr-plutus", RsSyntax.MkModuleName "LambdaBuffers.Runtime.Plutus.LamVal", RsSyntax.MkValueName "caseIntE")
...
printCaseIntE caseIntVal cases otherCase = do
  caseIntERefDoc <- RsSyntax.printRsQValName <$> LV.importValue caseIntERef
...
printListE vals = do
  ...
  return $ "vec!" <> brackets (align (encloseSep mempty mempty (comma <> space) valDocs))
...
printCtorE ((_, tyN), (ctorN, _)) prodVals = do
  ...
  let ctorNDoc = RsSyntax.printQualifiedCtorName (withInfo tyN) (withInfo ctorN)
  if null prodDocs
...
printRecordE ((_, tyN), _) vals = do
  fieldDocs <- for vals $
    \((fieldN, _), val) ->
      let fieldNDoc = RsSyntax.printFieldName (withInfo fieldN)
       in do
            valDoc <- printValueE val
            return $ group $ fieldNDoc <+> equals <+> valDoc
  let ctorDoc = RsSyntax.printMkCtor (withInfo tyN)
...
printRefE ref = do
  ...
  RsSyntax.printRsQValName <$> LV.importValue qvn
