...
```
-}
printTyDef :: MonadPrint m => PC.TyDef -> m (Doc ann, Set Purs.QClassName, Set Purs.QValName)
printTyDef tyd@(PC.TyDef tyN tyabs _) = do
  (kw, absDoc) <- printTyAbs tyN tyabs
  case kw of
    SynonymTyDef -> return (group $ printTyDefKw kw <+> printTyName tyN <+> absDoc, mempty, mempty)
    NewtypeTyDef ->
      let (ntDrvDoc, cimps, vimps) = printNewtypeDerive tyd
          (gDrvDoc, cimps', vimps') = printGenericDerive tyd
          (sInstDoc, cimps'', vimps'') = printShowInstance tyd
       in return
            ( vsep
    ...
    DataTyDef ->
      let (gDrvDoc, cimps, vimps) = printGenericDerive tyd
          (sInstDoc, cimps', vimps') = printShowInstance tyd
       in return
            ( vsep
                [ group $ printTyDefKw kw <+> printTyName tyN <+> absDoc
