    ...
    testGroup
      ...
      , testCase " ?- eq(maybe(var('A')))." $
          succeedsWith
            ...
            [ Struct
                "eq"
                [ Struct "maybe" [Struct "var" [Atom "A"]]
                ]
            ]
            mempty
      , testCase " ?- eq(maybe(var(A)))." $
...
succeedsWith clauses goals wanted = case solve clauses goals of
...
failsWith clauses goals wanted = case solve clauses goals of
  Left err -> assertEqual "Errors should match" err wanted
  Right sols -> assertFailure $ show ("Wanted an error but got" :: String, sols)
