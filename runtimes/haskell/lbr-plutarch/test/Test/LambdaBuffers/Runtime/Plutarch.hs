      ...
      [ testProperty "forall xs :: [Integer] ys :: [Integer]. (xs == ys) === evalEq (plistFrom xs) (plistFrom ys)" $
          H.property $
            H.forAll
              ((,) <$> genInts <*> genInts)
              >>= ( \(xs, ys) -> do
                      b <- liftIO $ evalBool (Lb.plistFrom @PList (pconstantInteger <$> xs) #== Lb.plistFrom (pconstantInteger <$> ys))
                      (xs == ys) H.=== b
                  )
            ...
            H.forAll
              ...
              >>= ( \xs -> do
                      b <-
                        liftIO $
                          evalBool
                            (plet (Lb.plistFrom @PList $ pconstantInteger <$> xs) $ \xs' -> pelimList (\x t -> pcons # x # t) pnil xs' #== xs')
                      True H.=== b
                  )
      ]
  where
